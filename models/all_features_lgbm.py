...
EXEC_TIME = 'all_features-lgbm-' + datetime.now().strftime("%Y%m%d-%H%M%S")
...
def load_data():
    ...
    X_train1 = pd.read_table(root + 'X_train.txt', sep=' ', header=None)
    X_train2 = pd.read_table(root + 'ECDF_Iwasawa_X_train.txt', sep=' ', header=None)
    X_train = pd.DataFrame(np.concatenate((X_train1, X_train2), 1))
    ...
    X_test1 = pd.read_table(root + 'X_test.txt', sep=' ', header=None)
    X_test2 = pd.read_table(root + 'ECDF_Iwasawa_X_test.txt', sep=' ', header=None)
    X_test = pd.DataFrame(np.concatenate((X_test1, X_test2), 1))
